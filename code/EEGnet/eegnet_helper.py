...
def plot_retr_accuracy(metrics, filename):
    fig = plt.figure(figsize=(20, 10))
    nsub,niter,_ = metrics.shape
    accuracy = metrics[:,:,0]

    for sub in range(nsub): 
        plt.plot(accuracy[sub], label='Sub {}'.format(sub+1))

    plt.plot(accuracy.mean(axis=0),linewidth=2,color='red')      
    plt.grid()
    fig.savefig('{}_superpos_retr.png'.format(filename), bbox_inches='tight')
    plt.close('all')
